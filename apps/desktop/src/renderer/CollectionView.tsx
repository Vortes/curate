...
function CollectionContent({ collectionId, onBack }: CollectionViewProps) {
	...
	const [selectedCapture, setSelectedCapture] =
		useState<CaptureCardData | null>(null)
	...
	const { data: collection, isLoading } = trpc.collection.byId.useQuery({
		id: collectionId,
	})
	...
	const { data: captureCollections = {} } =
		trpc.collection.captureCollections.useQuery(
			{ captureIds: selectedIdsArray },
			{ enabled: showOrganizeModal && selectedIdsArray.length > 0 },
		)

	const collectionsForOrganize: CollectionForOrganize[] = collectionsData.map(
		(c) => ({
			id: c.id,
			name: c.name,
			color: c.color,
	...
	const captures: CaptureCardData[] = (collection?.captures ?? []).map(
		(row) => ({
			id: row.capture.id,
	...
	const displayedCaptures = useMemo(() => {
		...
		return localOrder
			.map((id) => map.get(id))
			.filter((c): c is CaptureCardData => c !== undefined)
	}, [captures, localOrder])
			...
			<div className="flex-1 overflow-y-auto px-10 pt-3 pb-10 scrollbar-thin">
				...
				{captures.length === 0 ? (
					<div className="flex flex-col items-center justify-center gap-3 py-20 text-ink-quiet">
						...
						<p className="text-[12px] text-ink-whisper">
							Use Organize mode to add captures
						</p>
					</div>
				) : (
					<CaptureGrid
						...
						onDelete={
							isOrganizing
								? undefined
								: (captureId) => deleteCapture.mutate({ id: captureId })
						}
...
export function CollectionView({ collectionId, onBack }: CollectionViewProps) {
	return (
		<OrganizeModeProvider
			context={{ type: "collection", collectionId, collectionName: "" }}
		>
			<CollectionContent
